...
use crate::{AppState, auth::AuthContext, error::ApiError, validate::{self, validate}};
use anyhow::Context;
...
pub async fn set_wiki_permissions(
    ...
    let mut validate_lists = |map: &BTreeMap<String, Vec<String>>, allowed_flag: bool| -> Result<(), ApiError> {
        for (grp, list) in map {
    ...
    php.push_str(&format!("// Generated at {}Z\n", Utc::now().format("%Y-%m-%dT%H:%M:%S")));
    let mut entries: Vec<(&String,&String,&bool)> = combined.iter().map(|((g,p),a)| (g,p,a)).collect();
...
async fn wiki_info(slug: &str, db: &sqlx::Pool<sqlx::MySql>) -> sqlx::Result<Option<(u64, u64)>> {
    ...
    Ok(wiki_row.map(|row| {
        (row.get::<u64,_>("id"), row.get::<u64,_>("owner_user_id"))
    }))
}
...
pub async fn get_permissions(
    ...
    let wiki_id = sqlx::query_scalar::<_, u64>("SELECT id FROM wikifarm_wikis WHERE slug=? LIMIT 1")
        .bind(&slug)
...
async fn sync_subdirs(
    ...
    for item in items {
        ...
        if !tokio::fs::metadata(&src).await?.is_dir() {
            return Err(ApiError::new(StatusCode::BAD_REQUEST, "invalid_param", "extension dir not exist"));
        }
