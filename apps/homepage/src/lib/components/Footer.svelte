<script lang="ts">
	import favicon from '$lib/assets/favicon.svg';
	import { base } from '$app/paths';

	const footerLinks = {
		product: [
			{ label: 'Studio', href: 'https://studio.pub.wiki' },
			{ label: 'Hub', href: 'https://hub.pub.wiki' },
			{ label: 'Pricing', href: '/pricing' },
			{ label: 'Changelog', href: '/changelog' }
		],
		resources: [
			{ label: 'Documentation', href: '/docs' },
			{ label: 'API Reference', href: '/docs/api' },
			{ label: 'Examples', href: '/examples' },
			{ label: 'Blog', href: '/blog' }
		],
		community: [
			{ label: 'Discord', href: 'https://discord.gg/pubwiki' },
			{ label: 'GitHub', href: 'https://github.com/pubwiki' },
			{ label: 'Twitter', href: 'https://twitter.com/pubwiki' },
			{ label: 'Reddit', href: 'https://reddit.com/r/pubwiki' }
		],
		company: [
			{ label: 'About', href: '/about' },
			{ label: 'Careers', href: '/careers' },
			{ label: 'Privacy', href: '/privacy' },
			{ label: 'Terms', href: '/terms' }
		]
	};

	const socialLinks = [
		{
			name: 'GitHub',
			href: 'https://github.com/pubwiki',
			icon: 'M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z'
		},
		{
			name: 'Discord',
			href: 'https://discord.gg/pubwiki',
			icon: 'M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z'
		},
		{
			name: 'Twitter',
			href: 'https://twitter.com/pubwiki',
			icon: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z'
		}
	];
</script>

<footer class="bg-[#f6f8fa] border-t border-gray-200">
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
		<!-- Main Footer Content -->
		<div class="grid grid-cols-2 md:grid-cols-5 gap-8">
			<!-- Logo & Description -->
			<div class="col-span-2 md:col-span-1">
				<a href="{base}/" class="flex items-center gap-2 font-bold text-lg mb-4">
					<img src={favicon} alt="Pub.Wiki" class="h-8 w-8" />
					<span class="text-[#24292f]">Pub.Wiki</span>
				</a>
				<p class="text-sm text-[#57606a] mb-4">
					The open platform for AI-powered interactive fiction.
				</p>
				<!-- Social Links -->
				<div class="flex gap-3">
					{#each socialLinks as social (social.name)}
						<a
							href={social.href}
							target="_blank"
							rel="noopener"
							class="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-gray-500 hover:text-[#0969da] hover:border-[#0969da] transition-colors"
							title={social.name}
						>
							<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
								<path d={social.icon} />
							</svg>
						</a>
					{/each}
				</div>
			</div>

			<!-- Product -->
			<div>
				<h3 class="font-semibold text-[#24292f] mb-4">Product</h3>
				<ul class="space-y-2">
					{#each footerLinks.product as link (link.label)}
						<li>
							<a href={link.href} class="text-sm text-[#57606a] hover:text-[#0969da] transition-colors">
								{link.label}
							</a>
						</li>
					{/each}
				</ul>
			</div>

			<!-- Resources -->
			<div>
				<h3 class="font-semibold text-[#24292f] mb-4">Resources</h3>
				<ul class="space-y-2">
					{#each footerLinks.resources as link (link.label)}
						<li>
							<a href={link.href} class="text-sm text-[#57606a] hover:text-[#0969da] transition-colors">
								{link.label}
							</a>
						</li>
					{/each}
				</ul>
			</div>

			<!-- Community -->
			<div>
				<h3 class="font-semibold text-[#24292f] mb-4">Community</h3>
				<ul class="space-y-2">
					{#each footerLinks.community as link (link.label)}
						<li>
							<a href={link.href} class="text-sm text-[#57606a] hover:text-[#0969da] transition-colors">
								{link.label}
							</a>
						</li>
					{/each}
				</ul>
			</div>

			<!-- Company -->
			<div>
				<h3 class="font-semibold text-[#24292f] mb-4">Company</h3>
				<ul class="space-y-2">
					{#each footerLinks.company as link (link.label)}
						<li>
							<a href={link.href} class="text-sm text-[#57606a] hover:text-[#0969da] transition-colors">
								{link.label}
							</a>
						</li>
					{/each}
				</ul>
			</div>
		</div>

		<!-- Bottom Bar -->
		<div class="mt-12 pt-8 border-t border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4">
			<p class="text-sm text-[#57606a]">
				© {new Date().getFullYear()} Pub.Wiki. Open source under MIT License.
			</p>
			<div class="flex items-center gap-4 text-sm text-[#57606a]">
				<a href="{base}/privacy" class="hover:text-[#0969da] transition-colors">Privacy</a>
				<span>·</span>
				<a href="{base}/terms" class="hover:text-[#0969da] transition-colors">Terms</a>
				<span>·</span>
				<a href="{base}/cookies" class="hover:text-[#0969da] transition-colors">Cookies</a>
			</div>
		</div>
	</div>
</footer>
