...
- `[Relationships]`: `- target_id(name): value` — who is connected to whom
...
3. **`decision`** (object): Keys are entity keys (`entity_1`, `entity_2`, ...). **Only include selected entities** — omitted entities are excluded.
   ...
   - `"docs"` (array of strings): Selected document IDs (using the `doc_X_Y` IDs from Entity-Document Mapping), with optional flag suffix (`T`/`W`/`U`)
     - `"doc_2_3T"` = doc_2_3 with T flag, `"doc_2_1"` = doc_2_1 with no flag
        ...
        for entityKey, value in pairs(data.decision) do
            ...
            if entityIdx then
                ...
                for _, docEntry in ipairs(value.docs or {}) do
                    -- 分离 flag 后缀: "doc_1_2T" → base="doc_1_2", flag="T"
