...
registerSystem({
    ...
    description = "Query relationship between two creatures",
    usage = "Query one-way relationship from source to target. Returns has_relationship, name, value.",
    inputs = Type.Object({
        source_creature_id = Type.String:desc("source creature ID"),
        target_creature_id = Type.String:desc("target creature ID"),
    }),
    ...
    execute = function(params)
        ...
        if not entityId then
            return {success = false, error = "Source creature not found: " .. params.source_creature_id}
        end
...
registerSystem({
    ...
    description = "Set or update relationship between two creatures",
    usage = "Set relationship name and value from source to target. One-way by default; is_mutual=true sets both directions. Existing relationship will be overwritten.",
    inputs = Type.Object({
        source_creature_id = Type.String:desc("source creature ID"),
        target_creature_id = Type.String:desc("target creature ID"),
        relationship_name = Type.String:desc("relationship name"),
        value = Type.Int:desc("relationship value"),
        is_mutual = Type.Optional(Type.Bool):desc("whether to set mutual relationship (default false, one-way)"),
    }),
    outputs = Type.Object({
        success = Type.Bool,
        error = Type.Optional(Type.String),
    ...
    execute = function(params)
        ...
        -- Set source -> target relationship
        ...
        if not relResult.found then
            return {success = false, error = "Relationship component not found"}
        end
        ...
        -- If mutual, also set target -> source relationship
        if params.is_mutual then
            local targetEntityId = getEntityIdByCreatureId(params.target_creature_id)
            if not targetEntityId then
                return {success = false, error = "Target creature not found: " .. params.target_creature_id}
            end
            ...
            if not targetRelResult.found then
                return {success = false, error = "Target creature's Relationship component not found"}
            end
