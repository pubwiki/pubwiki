...
interface ModalState {
  ...
  rewriteTargetTurnId: number | null
  rewriteUserOpinion: string
  ...
  openRewriteModal: (turnId: number) => void
  closeRewriteModal: () => void
  setRewriteUserOpinion: (opinion: string) => void
...
export const useModalStore = create<ModalState>((set, get) => ({
  ...
  // 重写岁月史书模态框
  openRewriteModal: (turnId) => set({
    rewriteModalOpen: true,
    rewriteTargetTurnId: turnId,
    rewriteUserOpinion: '',
  }),
  closeRewriteModal: () => set({ rewriteModalOpen: false }),
  setRewriteUserOpinion: (opinion) => set({ rewriteUserOpinion: opinion }),

  // 剧情事件模态框
  openEventsModal: () => set({ eventsModalOpen: true }),
  closeEventsModal: () => set({ eventsModalOpen: false }),
}))
