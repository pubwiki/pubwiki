...
import type { WorldSnapshot, CustomComponentDef, CreatureAttrField, TypeSchema, LogEntry, SettingDocument, EventEntry } from '../../api/types'
...
export const WorldEditor: React.FC<{
  ...
  // --- 剧情事件 CRUD ---
  const addEvent = () => {
    const newEvents = [...events, { event_id: '', title: '', summary: '', content: '' }]
    onChange({ ...world, Events: { events: newEvents } })
    setEditing({ type: 'event', index: newEvents.length - 1 })
  }
  const updateEvent = (index: number, patch: Partial<EventEntry>) => {
    const newEvents = [...events]
    newEvents[index] = { ...newEvents[index], ...patch }
    onChange({ ...world, Events: { events: newEvents } })
  }
  const removeEvent = (index: number) => {
    onChange({ ...world, Events: { events: events.filter((_, i) => i !== index) } })
    closeModal()
  }
  ...
  // 当前编辑的事件
  const editingEvent = editing?.type === 'event' ? events[editing.index] : null
  const editingEventIndex = editing?.type === 'event' ? editing.index : -1
  // 当前编辑的笔记
  ...
  return (
    <div className="paper-bento-scroll">
      <div className="paper-bento-grid">
        ...
        {/* ===== 剧情事件 - 每个事件一张小卡片 ===== */}
        <div className="paper-bento-card paper-bento-events">
          <div className="paper-bento-summary-header">
            <h3 className="section-title">🎬 {t('world.sections.events')}</h3>
            <button className="paper-bento-edit-btn" onClick={addEvent}>+ {t('world.addEvent')}</button>
          </div>
          {events.length > 0 ? (
            <div className="paper-bento-item-grid">
              {events.map((evt, i) => (
                <div key={i} className="paper-bento-mini-card" onClick={() => setEditing({ type: 'event', index: i })}>
                  <div className="paper-mini-card-title">🎬 {evt.title || evt.event_id || '?'}</div>
                  <div className="paper-mini-card-desc">{evt.summary ? (evt.summary.length > 80 ? evt.summary.slice(0, 80) + '…' : evt.summary) : t('world.emptyEvent')}</div>
                  <div className="paper-bento-chips" style={{ marginTop: '4px' }}>
                    <span className="paper-bento-chip chip-blue" style={{ fontSize: '0.65rem' }}>{evt.event_id}</span>
                    {evt.related_entities && evt.related_entities.length > 0 && (
                      <span className="paper-bento-chip chip-pink" style={{ fontSize: '0.65rem' }}>👥{evt.related_entities.length}</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="paper-bento-empty">{t('world.noEvents')}</div>
          )}
        </div>
      ...
      {/* 单个事件 */}
      {editingEvent && (
        <BentoEditModal open onClose={closeModal} title={`🎬 ${editingEvent.title || editingEvent.event_id || t('world.newEvent')}`} size="wide">
          <div className="form-grid" style={{ marginBottom: '16px' }}>
            <div className="form-group">
              <label>{t('world.eventId')}</label>
              <input
                type="text"
                value={editingEvent.event_id || ''}
                onChange={e => updateEvent(editingEventIndex, { event_id: e.target.value })}
                placeholder="YYYY_MM_DD_ShortDesc"
                style={{ fontFamily: 'monospace' }}
              />
            </div>
            <div className="form-group">
              <label>{t('world.eventTitle')}</label>
              <input
                type="text"
                value={editingEvent.title || ''}
                onChange={e => updateEvent(editingEventIndex, { title: e.target.value })}
              />
            </div>
          </div>
          <div className="form-group" style={{ marginBottom: '16px' }}>
            <label>{t('world.eventSummary')}</label>
            <textarea
              value={editingEvent.summary || ''}
              onChange={e => updateEvent(editingEventIndex, { summary: e.target.value })}
              rows={3}
              placeholder={t('world.eventSummaryPlaceholder')}
            />
          </div>
          <div className="form-group" style={{ marginBottom: '16px' }}>
            <label>{t('world.eventContent')}</label>
            <textarea
              value={editingEvent.content || ''}
              onChange={e => updateEvent(editingEventIndex, { content: e.target.value })}
              rows={14}
              style={{ fontFamily: 'var(--font-mono, monospace)', fontSize: '0.85rem', lineHeight: '1.6' }}
            />
          </div>
          <div className="form-group" style={{ marginBottom: '20px' }}>
            <label>{t('world.eventRelatedEntities')}</label>
            <input
              type="text"
              value={(editingEvent.related_entities || []).join(', ')}
              onChange={e => updateEvent(editingEventIndex, {
                related_entities: e.target.value.split(',').map(s => s.trim()).filter(Boolean)
              })}
              placeholder="player_001, npc_jane, border_region, ..."
            />
          </div>
          <div style={{ textAlign: 'right' }}>
            <button className="btn-delete" onClick={() => removeEvent(editingEventIndex)}>{t('common:delete')}</button>
          </div>
        </BentoEditModal>
      )}
