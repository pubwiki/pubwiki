...
/**
 * Custom Zod type for attrs: accepts either a JSON string (from structured output)
 * or an already-parsed object. Validates that the parsed result is a record of
 * string keys → number/string values.
 */
const AttrsSchema = z.union([
    // Case 1: Already an object (record)
    z.record(z.string(), z.union([z.number(), z.string()])),
    // Case 2: JSON string from structured output (JSON Schema uses type: 'string')
    z.string().refine((s) => {
        try {
    ...
    }, { message: 'attrs must be a valid JSON string representing an object of attribute key-value pairs, e.g. \'{"strength": 75, "intelligence": 60}\'. Do NOT put field names or other non-JSON content here.' }),
])
...
/** Accepts object or JSON string (structured output uses type: 'string' for flexible keys) */
const FlexibleDataSchema = z.union([
    ...
    }, { message: 'data must be a valid JSON string representing an object, e.g. \'{"level": 3, "decay": 0.5}\'' }),
])
...
function validateCreaturesSemantics(output: any, session: WBNSession): { errors: string[], warnings: string[] } {
    ...
    for (const creature of creatures) {
        ...
        // 5. Validate attrs keys (attrs may be a JSON string or object)
        if (attrs?.attrs && validAttrFields.size > 0) {
            let attrsObj = attrs.attrs
