...
import { formatSettingChange, formatEventChange, type ServiceCallRecord } from '../../../api/types'
...
export function CollectorResultsSection({ turn }: { turn: StoryTurn }) {
  const { t } = useTranslation('game')
  const hasCollector = turn.collectorResults && turn.collectorResults.length > 0
  const hasEvents = turn.selectedEvents && turn.selectedEvents.length > 0
  if (!hasCollector && !hasEvents) return null

  const sectionId = `collector-${turn.id}`
  const autoCollapsed = turn.generationPhase === 'done' || turn.generationPhase === 'writing'
  const selectedCount = turn.collectorResults?.filter((r: CollectorResult) => r.selected).length || 0
  ...
  const eventCount = turn.selectedEvents?.length || 0
  ...
  return (
    <CollapsibleSection
      ...
      icon="🎯"
      badge={t('ink.sections.ragBadge', { selectedCount, totalDocs }) + (eventCount > 0 ? `, ${eventCount} ${t('ink.sections.ragEvents')}` : '')}
      generatingIndicator={turn.generationPhase === 'collecting'}
      className="collector-section"
    >
      {turn.collectorOutline && (
        <div className="collector-outline">{turn.collectorOutline}</div>
      )}
      ...
      {hasEvents && (
        <div className="collector-item selected" style={{ borderLeftColor: 'var(--paper-sunset-orange, #e67e22)' }}>
          <div className="collector-entity">🎬 {t('ink.sections.selectedEvents')}</div>
          <div className="collector-docs">
            {turn.selectedEvents!.map((eventId, idx) => (
              <div key={idx} className="doc-item selected">
                <div className="doc-path">🎬 {eventId}</div>
              </div>
            ))}
          </div>
        </div>
...
// 首次尝试失败记录（重试后显示）
function FirstAttemptSection({ firstAttempt }: { firstAttempt?: { calls?: ServiceCallRecord[]; results?: ServiceCallRecord[]; outline?: string; failCount?: number } }) {
  if (!firstAttempt) return null
  return (
    <div className="error-detail" style={{ opacity: 0.7 }}>
      <strong>⚠️ 首次尝试失败（{firstAttempt.failCount} 条失败，已重试）</strong>
      {firstAttempt.outline && <pre className="call-args">{firstAttempt.outline}</pre>}
      {firstAttempt.results?.map((r, idx) => (
      ...
      <div className="collapsible-section update-section error">
        ...
        {!collapsed && (
          <div className="collapsible-content error-content">
            ...
            <FirstAttemptSection firstAttempt={result.first_attempt} />
          </div>
        )}
      </div>
    )
