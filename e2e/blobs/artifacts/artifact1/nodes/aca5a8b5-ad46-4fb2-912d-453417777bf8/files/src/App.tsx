...
type GameType = 'INK' | 'TEST' | 'CUSTOM_TEMPLATE' | 'CUSTOM'
    ...
    const initSystem = async () => {
      try {
        ...
        window.LoadStateToGame = async (data) => {
          return await window.callService('state:LoadStateToGame', { data })
        }
  ...
  const renderGame = () => {
    switch (activeGame) {
      ...
      case 'CUSTOM_TEMPLATE':
        return <CustomTemplateGame onBack={() => setShowEditorOverlay(true)} />
      case 'CUSTOM':
        return (
          <Suspense fallback={<div>Loading...</div>}>
            <CustomGame onBack={() => setShowEditorOverlay(true)} />
          </Suspense>
        )
      default:
        return null
          ...
          <div className="editor-overlay">
            <div className="editor-overlay-content">
              <StateDataEditor
                ...
                onPublishApp={handlePublishApp}
                disableImport={false}
                onLaunchGame={async () => setShowEditorOverlay(false)}
              />
            </div>
          </div>
        )}
