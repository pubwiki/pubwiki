...
export function useHighlightEntities() {
  ...
  const highlightEntitiesInText = useCallback((text: string, keyPrefix: string) => {
    ...
    // 为含 · 的实体名生成别名（拆分后的各部分）
    // 仅当某个部分在所有实体中唯一属于一个实体时才作为别名
    const allNames = new Set(entities.map(e => e.name))
    const partOwnership = new Map<string, Set<string>>() // part -> Set<ownerFullName>
    for (const e of entities) {
      if (e.name.includes('·')) {
        for (const part of e.name.split('·')) {
          const trimmed = part.trim()
          if (trimmed.length < 2 || allNames.has(trimmed)) continue
          if (!partOwnership.has(trimmed)) partOwnership.set(trimmed, new Set())
          partOwnership.get(trimmed)!.add(e.name)
        }
      }
    }
    for (const [part, owners] of partOwnership) {
      if (owners.size === 1) {
        const owner = entities.find(e => e.name === owners.values().next().value)
        if (owner) {
          entities.push({ ...owner, name: part })
        }
      }
    }
