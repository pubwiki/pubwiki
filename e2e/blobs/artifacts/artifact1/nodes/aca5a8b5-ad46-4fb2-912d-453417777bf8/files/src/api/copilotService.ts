...
const COPILOT_FILES_STORE = 'copilot-files'  // Copilot-specific files, separate from WorldBuilder

// In-memory caches (sync reads, async persistence)
let _sessionsCache: CopilotChatSession[] = []
let _memoriesCache: StoredMemory[] = []
let _filesCache: UploadedFile[] = []        // WorldBuilder/WelcomePage shared files
let _copilotFilesCache: UploadedFile[] = [] // Copilot-only files
...
function openCopilotDB(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(COPILOT_FILES_STORE)) {
        db.createObjectStore(COPILOT_FILES_STORE, { keyPath: 'name' })
      }
    }
  })
}
...
export async function initCopilotDB(): Promise<{ sessions: CopilotChatSession[]; memories: StoredMemory[]; files: UploadedFile[] }> {
  ...
  try {
    ...
    _filesCache = files
    _copilotFilesCache = copilotFiles
...
// ============================================================================
// Copilot-Specific File Management (separate from WorldBuilder files)
// ============================================================================

/**
 * Load Copilot-only uploaded files from in-memory cache
 */
export function loadCopilotFiles(): UploadedFile[] {
  return _copilotFilesCache
}

/**
 * Add a file to Copilot workspace
 */
export function addCopilotFile(file: UploadedFile): UploadedFile[] {
  const existingIndex = _copilotFilesCache.findIndex(f => f.name === file.name)
  if (existingIndex >= 0) {
    _copilotFilesCache[existingIndex] = file
  } else {
    _copilotFilesCache.push(file)
  }
  idbPut(COPILOT_FILES_STORE, file)
  return _copilotFilesCache
}

/**
 * Remove a file from Copilot workspace
 */
export function removeCopilotFile(filename: string): UploadedFile[] {
  _copilotFilesCache = _copilotFilesCache.filter(f => f.name !== filename)
  idbDelete(COPILOT_FILES_STORE, filename)
  return _copilotFilesCache
}

/**
 * Clear all Copilot workspace files
 */
export function clearCopilotFiles(): void {
  _copilotFilesCache = []
  idbClear(COPILOT_FILES_STORE)
}
...
import {
  BUILTIN_WORKFLOW,
  BUILTIN_EDITING_RECIPES,
  BUILTIN_ADVANCED_TUNING,
} from './copilotPrompt'

// 内置 Skill IDs
export const BUILTIN_SKILL_IDS = {
  WORKFLOW: 'builtin_workflow',
  EDITING_RECIPES: 'builtin_editing_recipes',
  ADVANCED_TUNING: 'builtin_advanced_tuning'
} as const
...
function getBuiltinSkillsConfig(): Array<{
  ...
  return [
    ...
    {
      id: BUILTIN_SKILL_IDS.EDITING_RECIPES,
      title: t('builtinSkills.editingRecipesTitle', { ns: 'copilot', defaultValue: '[Built-in] Editing Recipes' }),
      description: t('builtinSkills.editingRecipesDesc', { ns: 'copilot', defaultValue: 'Common modification operations with code examples (characters, items, documents, regions, etc.)' }),
      content: BUILTIN_EDITING_RECIPES
    },
    {
      id: BUILTIN_SKILL_IDS.ADVANCED_TUNING,
      title: t('builtinSkills.advancedTuningTitle', { ns: 'copilot', defaultValue: '[Built-in] Advanced Tuning' }),
      description: t('builtinSkills.advancedTuningDesc', { ns: 'copilot', defaultValue: 'CustomComponent operations, condition-driven documents, batch modifications' }),
      content: BUILTIN_ADVANCED_TUNING
    }
  ]
}
