...
function generateStateOverview(state: StateData): string {
  // 深拷贝并截断长文本，StoryHistory 只保留元信息摘要
  const slim: Record<string, unknown> = {}
  for (const [k, v] of Object.entries(state)) {
    if (k === 'StoryHistory') {
      const history = v as import('../../api/types').StoryHistoryEntry[] | undefined
      if (history && history.length > 0) {
        slim[k] = {
          _summary: `共 ${history.length} 条剧情历史（使用 get_state_content("StoryHistory") 获取完整内容）`,
          count: history.length,
          turn_ids: history.map(e => e.turn_id),
          latest_turn_id: history[history.length - 1].turn_id,
          has_checkpoints: history.filter(e => e.story?.checkpoint_id).map(e => ({
            turn_id: e.turn_id,
            checkpoint_id: e.story.checkpoint_id
          }))
        }
      } else {
        slim[k] = { _summary: '暂无剧情历史', count: 0 }
      }
      continue
    }
