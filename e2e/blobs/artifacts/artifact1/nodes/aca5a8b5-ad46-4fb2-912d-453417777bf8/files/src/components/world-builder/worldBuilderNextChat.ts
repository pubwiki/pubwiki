...
async function extractForPhase(
    ...
    const extractionInstruction = getExtractionPrompt(phaseId, draft, story, session.initialPrompt, session.userDesignNotes)
...
/**
 * Build raw content from file/lorebook WITHOUT sub-agent extraction.
 * Injects the full content directly for the primary model to consume.
 */
function buildDirectContent(session: WBNSession): string {
    const parts: string[] = []
    ...
    if (lorebook) {
        ...
        if (enabledEntries.length > 0) {
            parts.push(`### Entries (${enabledEntries.length})`)
            for (const entry of enabledEntries) {
                const keysStr = entry.keys.length > 0 ? ` [keys: ${entry.keys.join(', ')}]` : ''
                parts.push(`--- Entry #${entry.id}: ${entry.comment}${keysStr} ---\n${entry.content}`)
            }
...
async function* runSynopsisPhase(
    ...
    // Step 1: Pre-extract file summary (or inject directly if skipExtraction)
    yield { type: 'extraction_progress', message: 'Analyzing reference materials...' }
    const extractedSummary = session.skipExtraction
        ? buildDirectContent(session)
        : await extractForPhase(config, session, 'synopsis')
    ...
    draftMessages.push({ role: 'user', content: getPhaseUserInstruction('synopsis', session.gameStyle, { skipOrganizations: session.skipOrganizations }) })
...
async function* runDataPhase(
    ...
    // Step 1: Pre-extract (or inject directly if skipExtraction)
    yield { type: 'extraction_progress', message: 'Extracting relevant content from reference materials...' }
    const extractedContent = session.skipExtraction
        ? buildDirectContent(session)
        : await extractForPhase(config, session, phaseId)
...
export async function* streamWBNRevision(
    ...
    for (let round = 0; round < MAX_ROUNDS; round++) {
        ...
        try {
            ...
            // Accumulate streaming response: text content + tool calls
            let assistantText = ''
            const toolCallAccum: Map<number, { id: string; name: string; arguments: string; thought_signature?: string }> = new Map()
                    ...
                    for (const tcDelta of delta.tool_calls) {
                        ...
                        if (!toolCallAccum.has(idx)) {
                            toolCallAccum.set(idx, {
                                ...
                                ...((tcDelta as any).thought_signature ? { thought_signature: (tcDelta as any).thought_signature } : {}),
                            })
                        } else {
                            ...
                            if ((tcDelta as any).thought_signature) existing.thought_signature = (tcDelta as any).thought_signature
                        }
                    }
                }

                // Emit streaming progress for UI feedback
                const totalChars = assistantText.length + Array.from(toolCallAccum.values()).reduce((s, tc) => s + tc.arguments.length, 0)
                if (lastProgressAt < 0 || totalChars - lastProgressAt >= 200) {
                    lastProgressAt = totalChars
                    yield { type: 'streaming_progress', charCount: totalChars }
                }
            }

            // Build assembled tool calls
            const toolCalls = Array.from(toolCallAccum.values())
                .filter(tc => tc.name && tc.id)
                .map(tc => ({ id: tc.id, type: 'function' as const, function: { name: tc.name, arguments: tc.arguments }, ...(tc.thought_signature ? { thought_signature: tc.thought_signature } : {}) }))
