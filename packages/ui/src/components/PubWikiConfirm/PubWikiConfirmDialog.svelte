<script lang="ts">
	/**
	 * PubWikiConfirmDialog - Shared confirmation dialog component
	 *
	 * Full-screen overlay + portal rendering.
	 * Accepts caller-provided form component and labels via props.
	 */
	import { fade } from 'svelte/transition';
	import { untrack } from 'svelte';
	import type { Component } from 'svelte';
	import type { FormComponentProps, ConfirmationType } from './pubwiki-confirm.svelte';
	import type { PubWikiConfirmDialogLabels } from './types';
	...
	interface Props {
		...
		onCancel: () => void;
		/** UI labels (i18n strings passed from app) */
		labels: PubWikiConfirmDialogLabels;
	}

	let { type, FormComponent, initialValues, onConfirm, onCancel, labels }: Props = $props();
...
>
	...
	<div 
		class="dialog-container bg-white rounded-xl shadow-2xl max-w-4xl w-full mx-4 overflow-y-auto"
	...
	>
		<div class="p-6">
			...
			<p class="text-gray-600 mb-4">{labels.description}</p>

			<!-- Action type -->
			<div class="text-sm font-medium text-gray-700 mb-3">
				{labels.actionLabel(type)}
			</div>
			...
			<p class="text-sm text-amber-600 mb-6">{labels.warning}</p>
