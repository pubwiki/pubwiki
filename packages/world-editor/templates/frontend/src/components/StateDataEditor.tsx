...
import CustomGame from '../games/custom/index'
...
import { loadWBNSession, getCurrentSessionId, startNewSession, endSession as endWBNSession, mergeReferenceFiles, mergeLorebooks, uploadedFilesToReferenceFiles } from '../api/worldBuilderNextService'
...
export const StateDataEditor: React.FC<EditorProps> = ({
  ...
  const [activeGame, setActiveGame] = useState<'custom' | 'test' | 'novel' | 'ink' | 'galgame' | null>(null)
  ...
  const handleBackToEditor = useCallback(async () => {
    ...
    if (result.success && result.data) {
      setData(result.data)
      savedDataRef.current = result.data
  ...
  // 发布应用（指定 publish_type）
  const handlePublishAs = useCallback(async (publishType: 'EDITOR' | 'INK') => {
    if (!onPublishApp) {
  ...
  // 获取当前 AppInfo 类型
  const [currentAppType, setCurrentAppType] = useState<string>('EDITOR')
  useEffect(() => {
    window.GetAppInfo?.().then(result => {
      if (result.success && result.data?.publish_type) {
        setCurrentAppType(result.data.publish_type.toUpperCase())
      }
    }).catch(() => {})
  }, [])
  ...
  const handleExport = useCallback(async () => {
    ...
    if (format === 'json') {
      const blob = new Blob([JSON.stringify(denormalizeLuaData(exportData), null, 2)], { type: 'application/json' })
      ...
      try {
        ...
        const pngBlob = await encodeDataToPng(denormalizeLuaData(exportData), customImage)
    ...
    input.onchange = async (e) => {
      ...
      try {
        if (format === 'json') {
          ...
          if (!isV2SaveData(imported)) {
            showAlert(t('file.incompatibleVersion'))
            return
          }
          migrateStateData(imported)
          setData(imported)
          showToast(t('common:importSuccess'), 'success')
        } else {
          const decoded = await decodeDataFromPng(file)
          if (decoded) {
            if (!isV2SaveData(decoded)) {
              showAlert(t('file.incompatibleVersion'))
              return
            }
            migrateStateData(decoded as Record<string, any>)
            setData(decoded as StateData)
            showToast(t('common:importSuccess'), 'success')
          } else {
    ...
    <div className="paper-editor">
            ...
            <div className="dropdown-menu toolbar-desktop-only">
              ...
              {activeDropdown === 'app' && (
                <div className="dropdown-content">
                  {onPublishApp && (
                    <>
                      <button onClick={async () => { closeDropdown(); await handlePublishAs('EDITOR'); }}>
                        ✏️ {t('app.publishAsEditor')}
                      </button>
                      <button onClick={async () => { closeDropdown(); await handlePublishAs('INK'); }}>
                        📖 {t('app.publishAsInk')}
                      </button>
                      <div className="dropdown-divider"></div>
                    </>
        ...
        {activeGame === 'galgame' && (
          <div style={{
            ...
            <GalgameGame onBack={handleBackToEditor} />
          </div>
        )}
