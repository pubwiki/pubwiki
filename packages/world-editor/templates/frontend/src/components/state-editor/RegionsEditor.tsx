                  ...
                  <div className="paper-bento-item-grid">
                    {paths.map((path, i) => (
                      <div key={`path-${i}`} className="paper-bento-mini-card" onClick={() => setEditing({ type: 'path', index: i })}>
                        <div className="paper-mini-card-title">
                          {getRegionName(path.src_region)} → {getRegionName(path.to_region)}
                        </div>
                        <div className="paper-mini-card-desc">{t('regions.pathNumber', { number: i + 1 })}</div>
                        <div className="paper-bento-chips" style={{ marginTop: '4px' }}>
                          {path.discovered && (
                            <span className="paper-bento-chip chip-active" style={{ fontSize: '0.65rem' }}>
                              {t('regions.discovered')}
                            </span>
                          )}
                        </div>
                      </div>
