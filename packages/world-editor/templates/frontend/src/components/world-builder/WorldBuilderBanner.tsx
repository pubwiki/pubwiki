...
import { denormalizeLuaData } from '../../utils/normalizeLuaData'
...
const WorldBuilderBanner: React.FC<WorldBuilderBannerProps> = ({
    ...
    const onLogEvent = useCallback((event: string) => {
        setLogEvents(prev => [...prev.slice(-200), event])
    }, [])
    ...
    const handleStartSession = useCallback(() => {
        ...
        const newSession = startNewSession(setupPrompt.trim(), mergedFile, mergedLorebook)
        onSessionChange(newSession)
        setMode('generating')
        setLogEvents([])
    }, [setupPrompt, setupFiles, setupLorebooks, onSessionChange, clearBeforeGenerate, onStateChange])
    ...
    const handlePhaseOutput = useCallback((phaseId: WBNPhaseId, output: unknown) => {
        ...
        if (phaseId !== 'synopsis') {
            const newState = applyPhaseToState(s, phaseId, stateRef.current)
                ...
                switch (event.type) {
                    ...
                    case 'revision_patch':
                        ...
                        // Re-apply current phase to state after patch
                        {
                            ...
                            if (currentOutput) {
                                const newState = applyPhaseToState(sessionRef.current, phaseId, stateRef.current)
                                onStateChange(newState)
                                stateRef.current = newState
                            }
                        }
            ...
            {showLog && logEvents.length > 0 && (
                <div className="wbn-banner-log">
                    {logEvents.map((event, i) => (
                        <div key={i} className="wbn-banner-log-line">{event}</div>
                    ))}
                    <div ref={logEndRef} />
                </div>
            )}
