...
export interface UpdateGameStateAndDocsOutput {
  ...
  audit?: string;
  outline?: string;
  calls?: Array<{ service: string; reason: string; args: any }>;
  results?: Array<{
    service: string;
    reason: string;
    success: boolean;
...
export interface UpdateGameStateAndDocsInput {
  new_event: string;
  state_changes: string[]; // 状态变化列表
  setting_changes: string[]; // 建议设定变化列表
...
export type AppInfo = {
  publish_type?: "EDITOR" | "NOVEL" | "INK" | "TEST" | "CUSTOM" | "GALGAME"; // 默认为 EDITOR
};
