...
Service:define()
    ...
    :desc("通用创意写作服务(流式版)，专门用于生成剧情文本。内部会自动处理 RAG 检索和 LLM 调用，调用方只需关注 output_content_schema 和 callback。")
    :usage([==[
创意写作流式服务。内部自动完成 RAG 筛选 → LLM 生成 → 流式输出。

【重要：调用方职责】
调用方只需要：
  1. 提供 create_request（创作请求）、thinking_instruction（思考指令）、previous_content_overview（前文概要）
  2. 提供 output_content_schema —— 仅描述你期望的【创意内容】结构（即 step_2 的输出格式）
  3. 提供 callback 函数接收流式事件

【重要：output_content_schema 只描述创意内容】
output_content_schema 只用于定义 step_2_creative_content 的结构，即你的业务内容格式。
例如：`{ novel_content: string; chapter_hint: string; player_choices: Array<{name: string}> }`
不要在 output_content_schema 中包含 step_3a/step_3b/step_3c/step_4 等内部字段 —— 这些由服务内部自动处理和输出。

【callback 事件流】
callback 按以下顺序接收事件：
  1. "collector_result_update" → RAG 检索完成
  2. "reasoning_update" → LLM 推理中（可能多次）
  3. "result_update" → 流式增量内容（可能多次，包含 partial JSON）
  4. "done" → 生成完成，包含最终完整结果
  如果出错则收到 "error" 事件。

【与 UpdateGameStateAndDocs 的关系】
本服务的 "done" 事件输出中包含 setting_changes / event_changes / new_entities / director_notes / updater_messages，
这些字段应传给 UpdateGameStateAndDocs 服务来更新游戏状态。调用方不需要自己构造这些字段。
]==])
    :inputs(
        Type.Object(
            {
                ...
                output_content_schema = Type.String:desc("【仅描述 step_2 创意内容的结构】使用 TypeScript 接口格式。例如：'{ novel_content: string; chapter_hint: string; player_choices: Array<{name: string; description: string}> }'。不要包含 step_3a/step_3b/step_3c/step_4 等内部字段，那些由服务自动处理。"),
                output_content_schema_definition = Type.Optional(Type.Object({})):desc("可选的 output_content_schema 对应的 JSON Schema 定义。如果提供，则会按照 schema 严格约束 step_2 输出结构。"),
                ...
                callback = Type.Function({params = Type.Object({
                    event_type = Type.String:desc([==[
事件类型，可能的值：

"collector_result_update": RAG 收集完成
  event_data: { collector_results: any[]; collector_outline: string; selected_events: any[] }

"reasoning_update": 推理阶段更新
  event_data: { raw_text: string; reasoning: string; collector_results: any[]; collector_outline: string }

"result_update": 流式增量内容（partial JSON 解析，字段可能为 undefined）
  event_data: {
    raw_text: string;           // LLM 原始输出文本
    reasoning: string;          // 推理过程文本
    thinking?: string;          // step_1: 思考结果
    content?: any;              // step_2: 创意内容（结构由 output_content_schema 决定）
    setting_changes?: any[];    // step_3a: 设定更新建议
    event_changes?: any[];      // step_3b: 事件更新建议
    new_entities?: any[];       // step_3c: 新增实体建议
    director_notes?: any;       // step_4: 导演笔记
    collector_results: any[];   // RAG 收集结果
    collector_outline: string;  // RAG 收集大纲
  }
...
"done": 生成完成
  event_data: {
    ...
    updater_messages: any[];    // 状态更新器消息列表
  }

"error": 错误
  event_data: string  // 错误描述

"debug_chunk": 调试用原始 chunk
  event_data: { type: string; token: string }
]==]),
                    event_data = Type.Any:desc("事件数据，结构取决于 event_type，详见 event_type 描述"),
                })}):desc("【必须是函数】流式回调函数，接收生成过程中的各阶段事件。不能传空对象 {}，必须传一个 function(params) 形式的回调。"),
            }
        )
    )
    :outputs(Type.Object({}))
