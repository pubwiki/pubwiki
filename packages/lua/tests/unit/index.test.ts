...
describe('pubwiki-lua', () => {
  ...
  describe('State:set', () => {
    ...
    it('should serialize and deserialize Lua tables as JSON', async () => {
      const result = await runLua(`
      ...
      // The result should be deserialized back to an object (JsProxy in Lua)
      expect(result.output).toContain('table')
      expect(result.error).toBeNull()
      // After deserialization, r should be an object with test=123 and v="456"
      expect(result.result).toEqual({ test: 123, v: '456' })
    })
    ...
    it('should serialize nested tables', async () => {
      const result = await runLua(`
        local nested = {
          name = "test",
          data = {
        ...
        State:set("a", ":nested", nested)
        local r = State:get("a", ":nested")
        return r
      `, { rdfStore: store })
    ...
    it('should allow assigning State:get result to a Lua table and return correctly', async () => {
      // First, store some data
      await runLua(`
        local docs = {
          {content = "Hello", path = "a/b"},
          {content = "World", path = "c/d"}
        }
        State:set("game:state", "setting_documents", docs)
      `, { rdfStore: store })

      // Test 1: Direct return works
      const directResult = await runLua(`
        local settingDocsResults = State:get("game:state", "setting_documents")
        return settingDocsResults
      `, { rdfStore: store })
      
      expect(directResult.error).toBeNull()
      expect(directResult.result).toEqual([
        { content: "Hello", path: "a/b" },
        { content: "World", path: "c/d" }
      ])

      // Test 2: Assign to table and return table.a (the JsProxy directly)
      const tableFieldResult = await runLua(`
        local result = {}
        local settingDocsResults = State:get("game:state", "setting_documents")
        result["a"] = settingDocsResults
        return result["a"]
      `, { rdfStore: store })
      
      expect(tableFieldResult.error).toBeNull()
      expect(tableFieldResult.result).toEqual([
        { content: "Hello", path: "a/b" },
        { content: "World", path: "c/d" }
      ])

      // Test 3: Return the whole table - this is the failing case
      const wholeTableResult = await runLua(`
        local result = {}
        local settingDocsResults = State:get("game:state", "setting_documents")
        result["a"] = settingDocsResults
        return result
      ...
      expect(wholeTableResult.error).toBeNull()
      expect(wholeTableResult.result).toEqual({
        a: [
          { content: "Hello", path: "a/b" },
          { content: "World", path: "c/d" }
        ]
