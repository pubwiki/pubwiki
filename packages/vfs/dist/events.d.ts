...
/** 所有 VFS 事件的联合类型 */
export type VfsEvent = FileCreatedEvent | FileUpdatedEvent | FileDeletedEvent | FileMovedEvent | FolderCreatedEvent | FolderUpdatedEvent | FolderDeletedEvent | FolderMovedEvent | VersionCommitEvent | VersionCheckoutEvent | VersionRevertEvent;
/** 事件类型字符串 */
export type VfsEventType = VfsEvent['type'];
/** 根据事件类型获取事件数据类型 */
export type VfsEventData<T extends VfsEventType> = Extract<VfsEvent, {
    type: T;
}>;
/** 事件处理函数类型 */
export type VfsEventHandler<T extends VfsEventType = VfsEventType> = (event: VfsEventData<T>) => void | Promise<void>;
/**
 * VFS 事件总线
 *
 * 提供文件系统事件的发布/订阅功能。
 */
export declare class VfsEventBus {
    private listeners;
    /**
     * 订阅特定类型的事件
     ...
     * @returns 取消订阅函数
     */
    on<T extends VfsEventType>(eventType: T, handler: VfsEventHandler<T>): () => void;
    /**
     * 订阅一次性事件
     * @param eventType 事件类型
     * @param handler 事件处理函数
     * @returns 取消订阅函数
     */
    once<T extends VfsEventType>(eventType: T, handler: VfsEventHandler<T>): () => void;
    /**
     * 取消订阅
     * @param eventType 事件类型
     * @param handler 事件处理函数
     */
    off<T extends VfsEventType>(eventType: T, handler: VfsEventHandler<T>): void;
    /**
     * 订阅所有事件
     * @param handler 事件处理函数
     * @returns 取消订阅函数
     ...
     * @param event 事件对象
     */
    emit<T extends VfsEventType>(event: VfsEventData<T>): Promise<void>;
    /**
     * 清除所有监听器
     */
    clear(): void;
