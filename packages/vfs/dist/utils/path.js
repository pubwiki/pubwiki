 ...
 * 从文件名和文件夹路径构建完整路径
 */
export function buildFilePath(folderPath, fileName) {
 ...
 * 从完整路径提取文件夹路径
 */
export function getFolderPath(path) {
 ...
 * 检查路径是否是另一个路径的子路径
 */
export function isSubPath(path, parentPath) {
 ...
 * 获取相对路径
 */
export function getRelativePath(path, basePath) {
    const normalizedPath = normalizePath(path);
