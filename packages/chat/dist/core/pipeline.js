...
export class ChatStreamPipeline {
    ...
    async *stream(messages) {
        ...
        for (let iteration = 0; iteration < maxIterations; iteration++) {
            ...
            let toolCalls = [];
            let segmentReasoningDetails = [];
            // Accumulate raw text for MessageBlock generation
            let pendingContent = '';
            let currentMessageBlockId = null;
                ...
                if (chunk.reasoning_details && chunk.reasoning_details.length > 0) {
                    for (const detail of chunk.reasoning_details) {
                        ...
                        if (detail.type === 'reasoning.encrypted' && detail.data) {
                            const existingIndex = segmentReasoningDetails.findIndex(d => d.id === detail.id && d.type === detail.type);
                            if (existingIndex === -1) {
            ...
            if (iteration + 1 >= maxIterations && toolCalls.length > 0) {
                ...
                if (this.config.onIterationLimitReached) {
                    try {
                        const shouldContinue = await this.config.onIterationLimitReached(iteration + 1, maxIterations);
                        if (shouldContinue) {
     ...
     * Merge tool calls
     */
    mergeToolCalls(existing, incoming) {
