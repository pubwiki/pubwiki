...
import { loadGraph, saveGraph, ensureDefaultProject } from './db';
...
/**
 * Create a persisted graph state (nodes + edges).
 * 
 * This is the main hook for Studio's flow editor, providing:
 * - Automatic loading from IndexedDB on mount
 * - Automatic saving with debouncing on changes
 * - Reactive state that works with @xyflow/svelte
 * 
 * @example
 * ```svelte
 * <script lang="ts">
 *   import { useGraphState } from './live-query.svelte';
 *   import type { StudioNodeData } from '../utils/types';
 *   
 *   const graph = useGraphState<StudioNodeData>('default');
 * </script>
 * 
 * <SvelteFlow 
 *   nodes={graph.nodes} 
 ...
 * ```
 * 
 * @param projectId - Project ID to load/save
 * @param options - Configuration options
 */
export function useGraphState<T extends NodeData>(
  projectId: string = 'default',
  options: { debounceMs?: number; autoSave?: boolean } = {}
): GraphStateResult<T> {
  let nodes = $state<Node<T>[]>([]);
  ...
  async function load() {
    try {
      ...
      const result = await loadGraph<T>(projectId);
      nodes = result.nodes;
      edges = result.edges;
      loaded = true;
      error = undefined;
      ...
      error = err instanceof Error ? err : new Error(String(err));
      loaded = true; // Still mark as loaded to allow new data
    }
  }
  ...
  return {
    ...
    updateNode(id: string, updater: (data: T) => T) {
      nodes = nodes.map(n => 
