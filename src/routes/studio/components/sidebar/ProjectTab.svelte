	...
	<form id="project-form" onsubmit={handleSubmit} class="p-4 space-y-4">
		...
		{#if !isDraft}
			<div class="rounded-lg bg-green-50 border border-green-200 p-3 flex items-start gap-3">
				...
				<div>
					<p class="text-sm font-medium text-green-800">Published</p>
					<p class="text-xs text-green-600 mt-0.5">This project has been published as an artifact.</p>
					<a
		...
		{#if !isAuthenticated}
			<div class="rounded-lg bg-amber-50 border border-amber-200 p-3 flex items-start gap-3">
				...
				<div>
					<p class="text-sm font-medium text-amber-800">Login required</p>
					<p class="text-xs text-amber-600 mt-0.5">Please login to publish your project.</p>
				</div>
			</div>
		{/if}
		...
		<div class="space-y-3">
			...
			<div>
				...
				<input
					...
					class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
				/>
				<p class="mt-1 text-xs text-gray-400">URL identifier (lowercase, numbers, hyphens)</p>
			</div>
			...
			<div class="grid grid-cols-2 gap-3">
				...
				<div>
					<label for="visibility" class="block text-xs font-medium text-gray-500 mb-1">Visibility</label>
					<select
						id="visibility"
			...
			<div>
				<label for="description" class="block text-xs font-medium text-gray-500 mb-1">Description</label>
				<textarea
					id="description"
			...
			<div>
				<label for="tags" class="block text-xs font-medium text-gray-500 mb-1">Tags</label>
				<input
					id="tags"
					type="text"
			...
			<div>
				<label for="homepage" class="block text-xs font-medium text-gray-500 mb-1">
					Homepage <span class="text-gray-400">(Markdown)</span>
				</label>
				<textarea
					id="homepage"
					bind:value={homepage}
		...
		<div class="border-t border-gray-100 pt-4">
			...
			{#if nodesToPublish.length === 0}
				<div class="text-center py-6 text-gray-400">
					...
					<p class="text-xs">No nodes to publish</p>
				</div>
			{/if}
		</div>
