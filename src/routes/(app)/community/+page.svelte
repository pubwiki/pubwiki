<script lang="ts">
	...
	function handleSortChange(e: Event) {
		...
		const option = sortOptions.find(o => o.label === target.value);
		if (option) {
...
<div class="mx-auto max-w-[1200px] px-4 py-6">
	<!-- Page Header -->
	<div class="mb-6">
		<h1 class="text-2xl font-bold text-gray-900 mb-2">Community Projects</h1>
		<p class="text-gray-600">Discover and explore curated collections of artifacts created by the community.</p>
	</div>
	...
	<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 bg-white p-4 rounded-md border border-gray-200 shadow-sm">
		<div class="flex items-center gap-4">
			<div class="flex items-center gap-2">
				<label for="topic-filter" class="text-sm font-medium text-gray-700">Topic:</label>
				<input
					id="topic-filter"
					type="text"
		...
		<div class="flex items-center gap-2">
			<label for="sort-select" class="text-xs text-gray-500">Sort by:</label>
			<select
				id="sort-select"
	...
	{:else if projectStore.projects.length === 0}
		<div class="text-center py-12 text-gray-500">
			...
			<p class="text-sm mt-1">Be the first to create a project!</p>
		</div>
	{:else}
						...
						<div class="max-w-2xl">
							...
							<div class="flex items-center gap-6 text-sm text-gray-300">
								...
								<div class="flex items-center gap-4 border-l border-gray-500/50 pl-4">
									...
									<span class="flex items-center gap-1.5" title="Maintainers">
										<svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
										</svg>
										{(project.maintainerCount ?? 0) + 1} maintainers
									</span>
								</div>
							</div>
						</div>
		...
		{#if projectStore.pagination && projectStore.pagination.totalPages > 1}
			<div class="flex justify-center items-center gap-2 mt-8">
				...
				<span class="text-sm text-gray-600">
					Page {projectStore.pagination.page} of {projectStore.pagination.totalPages}
				</span>
				<button
					onclick={() => projectStore.fetchProjects({ page: projectStore.pagination!.page + 1, sortBy, sortOrder, topic: topicFilter || undefined })}
