...
<a href="/game/{artifact.id}" class="flex gap-3 group">
	<div class="w-16 h-16 rounded overflow-hidden shrink-0 border border-gray-200">
		<img src={artifact.coverImage} alt={artifact.title} class="w-full h-full object-cover group-hover:opacity-80 transition" />
	</div>
	<div class="flex-1 min-w-0">
		<h4 class="text-sm font-bold text-gray-900 group-hover:text-[#0969da] truncate">{artifact.title}</h4>
		<p class="text-xs text-gray-500 truncate">by {artifact.owner_name}</p>
